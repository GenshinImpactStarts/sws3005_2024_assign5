              ...
              out bool hasHit, out vec3 hitPos, out vec3 hitNormal, out vec3 k_rg )
{
    ...
    for ( int i = 0; i < NUM_PLANES; i++ ) {
        if ( IntersectPlane( Plane[i], ray, DEFAULT_TMIN, nearest_t,
    ...
    for ( int i = 0; i < NUM_SPHERES; i++ ) {
        if ( IntersectSphere( Sphere[i], ray, DEFAULT_TMIN, nearest_t,
                              temp_t, temp_hitPos, temp_hitNormal ) ) {
            hasHitSomething = true;
            nearest_t = temp_t;
            nearest_hitPos = temp_hitPos;
    ...
    for ( int i = 0; i < NUM_LIGHTS; i++ ) {
        ...
        bool inShadow = false;
        float tmax = length( Light[i].position - nearest_hitPos );
        for ( int j = 0; !inShadow && j < NUM_PLANES; j++ ) {
            inShadow = IntersectPlane( Plane[j], shadowRay, DEFAULT_TMIN, tmax );
        }
        for ( int j = 0; !inShadow && j < NUM_SPHERES; j++ ) {
            inShadow = IntersectSphere( Sphere[j], shadowRay, DEFAULT_TMIN, tmax );
        }
