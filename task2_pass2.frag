...
{
    cam.pos = texelFetch( iChannel0, 0 ).xyz;
    cam.x   = texelFetch( iChannel0, 1 ).xyz;
    cam.y   = texelFetch( iChannel0, 2 ).xyz;
    cam.z   = texelFetch( iChannel0, 3 ).xyz;
}
...
void loadTriangle( in int triangleIndex, out Triangle_t tri )
{
    int index = triangleIndex * TRIANGLES_UNIT + TRIANGLES_OFFSET;
...
bool IntersectAABB( in Ray_t ray )
{
    vec3 invD = vec3(
    ...
    vec3 t_near = (AABB_MIN - ray.o) * invD;
    vec3 t_far = (AABB_MAX - ray.o) * invD;

    vec3 tmin = min(t_near, t_far);
    vec3 tmax = max(t_near, t_far);
              ...
              out bool hasHit, out vec3 hitPos, out vec3 hitNormal, out vec3 k_rg )
{
    ...
    if ( IntersectAABB( ray ) ) {
        for ( int i = 0; i < NUM_TRIANGLES; i++ ) {
            ...
            Barycentric_t bary;
            loadTriangle( i, tri );
            if ( IntersectTriangle( tri, ray, DEFAULT_TMIN, nearest_t,
                                    temp_t, temp_hitPos, temp_hitNormal, bary ) ) {
                hasHitSomething = true;
                nearest_t = temp_t;
                nearest_hitPos = temp_hitPos;
                nearest_hitNormal = temp_hitNormal;
                nearest_hitMatID = tri.materialID;
            }
        }
    }
    ...
    for ( int i = 0; i < NUM_LIGHTS; i++ ) {
        ...
        if ( IntersectAABB( shadowRay ) ) {
            for ( int j = 0; !inShadow && j < NUM_TRIANGLES; j++ ) {
                Triangle_t tri;
                loadTriangle( j, tri );
                inShadow = IntersectTriangle( tri, shadowRay, DEFAULT_TMIN, tmax );
            }
        }
